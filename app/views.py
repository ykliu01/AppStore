...
def findCalculators(request):
    ...
    if request.POST:
        with connection.cursor() as cursor:
            ...
            if user_input[0]==-1 and user_input[1]=='ANY' and user_input[2]=='ANY':
                select_statement = "SELECT c.brand, c.serial_number, c.price, c.calc_condition, l.location_name, s.time_availability, s.first_name, s.last_name, s.email FROM calculators c, students s, locations l WHERE c.availability='available' AND c.email = s.email AND l.location_id=s.location_id AND (CAST(%s as INTEGER)>=0) AND l.location_name<>%s AND c.calc_type<>%s"
